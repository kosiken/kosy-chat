...
function Others() {
  ...
  const init = async () => {
    ...
    if (!window.remoteConnection) window.remoteConnection = new RTCPeerConnection(null, {
      optional: [{
        RtpDataChannels: true
      }]
    });
    ...
    window.localChannel = localc = lc.createDataChannel('messaging-channels', dataChannelParams);
